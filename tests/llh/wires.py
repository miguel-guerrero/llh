...
class Wire:
    ...
    def setVal(self, x):
        ...
        if x != self.T or self.neverDriven:
            ...
            if self.vcdHandle is not None:
                val = x.V()
                xval = x.X()
                gSim.vcdChange(self.vcdHandle,
                               val if xval == 0 else 'x')
            self.T.setVal(x)
            # propagate to loads
    ...
    def __getitem__(self, slc):
        ...
        raise Exception(
            f"subscripting operator requires an integer, found {slc}")
...
class Out(Wire):
    ...
    def connect(self, drivingSubPort):
        if isinstance(drivingSubPort, Out):
            ...
            raise Exception(
                    f"cannot connect {drivingSubPort} to out port {self}")
